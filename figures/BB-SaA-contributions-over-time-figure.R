...
##=====================================================================##
# rm(list=ls(all=T)) 
pkgs<-c("tidyverse","dplyr","rlist","readxl","nlme","MuMIn","visreg","effects","RColorBrewer","brms","heavy","splitstackshape","pracma")
...
method<-"evl" ## early-vs-late ('evl') or linear model with AR1 ('gls')
...
if(dropAlagnak) data<-data[data$stock!= "Alagnak",]
years<-unique(data$year);nY<-length(years)

##===================================## recruitment (return by brood year)
recruits<-read_excel("/Users/janohlberger/Documents/Work/UW/SalmonSizeChange/DataBB/Brood-Return-Tables/2020_BBay_Brood_Tables_Recruitment.xlsx")
...
##--------------------------------------## from 'Sockeye4' photo by Jason
cols<-c("#C52C2E","#60ADB6")#,"#85501A")

##==================================## size-at-age-contribution by system
pdf("Size-at-age-contribution-over-time.pdf",width=8,height=6)
...
# ylim<-c(-20,20)
xlab<-paste0("Last ",year.type," year of analysis")
ylab<-"Size change (mm)"
...
##==================================## size-at-age-contribution by system
pdf("Size-at-age-contribution-over-time-with-recruits.pdf",width=8,height=6)
...
##----------------------------------------------------## add recruitment
# par(new=TRUE)
# x<-recruits$Brood.Year
# y<-recruits[,names(recruits)==river]
# ymax<-max(y)*1.1  
# plot(NA,NA,xlim=c(min(x),max(x)),ylim=c(0,ymax),type="l",axes=F,xaxt="n",yaxt="n",xlab="",ylab="")
# lines(x[-c(1:4)],movavg(y,5,"s")[-c(1:4)],lwd=rec.lwd,lty=rec.lty) 
##---------------------------------------------## end loop over systmes
} 
##-----------------------------------------------------------## save plot
